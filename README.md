...
<p>You can configure a list of effect status ids that prevent the aura from affecting targets and the owner (such as if the source of the aura is dead or unconscious). A 'status id' is a hidden and unique identifier that any status condition has, and the Keys button in the builder will help you pick one out from those that exist on the token HUD. The field itself is not validated; if you are able to give an effect a status id of your own choosing, that is respected as well; simply write the status id in the field.</p>
...
<p><strong><em>Creature Types.</em></strong> Filter the bonus to only apply if you are targeting an enemy belonging to one of the included creature types, such as 'undead', 'fey', or 'humanoid', while not targeting any of the excluded creature types.</p>
